...
func Code(fn interface{}) TestDeep {
	...
	if vfn.Kind() != reflect.Func {
		f := dark.GetFatalizer()
		f.Helper()
		f.Fatal(color.BadUsage("Code(FUNC)", fn, 1, true))
	}
	...
	if fnType.IsVariadic() || fnType.NumIn() != 1 {
		f := dark.GetFatalizer()
		f.Helper()
		f.Fatal(color.Bad("Code(FUNC): FUNC must take only one non-variadic argument"))
	}
	...
	f := dark.GetFatalizer()
	f.Helper()
	f.Fatal(color.Bad("Code(FUNC): FUNC must return bool or (bool, string) or error"))
